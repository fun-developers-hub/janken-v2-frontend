...
export default function Page() {
  return (
    <article>
      <h2 className="mb-4 text-2xl font-bold">Route Handler で作るAPIモック</h2>
      <TabSwitcher
        tabs={[
          {
            id: "explanation",
            ...
            content: (
              <div className="flex flex-col gap-3 text-sm leading-relaxed">
                <p>
                  ...
                  実装できる（Route Handler）。<code>GET</code>/
                  <code>POST</code> などHTTPメソッド名の関数をexportし、
                  <code>NextResponse.json()</code> でレスポンスを返す。
                </p>
                <p>
                  実際のバックエンド（
                  <code>https://janken.ma41.net</code>）のswagger仕様では、
                  <code>POST /janken</code> は <code>user_hand</code>{" "}
                  を受け取り、<code>cpu_hand</code> と <code>result</code>
                  （win/lose/draw）を返す。 モックもこの形に合わせて作ることで、
                  Phase 3で実APIに差し替える際の変更を小さくできる。
                </p>
                <p>
                  不正な <code>user_hand</code>{" "}
                  が送られてきた場合は400を返すようにしており、
                  これは次の「fetch
